...
const ERR_RENDERER_IDX_UNDEFINED = "Cannot switch the active renderer to %d. No renderers are registered at this index. Check your renderers_registry.tres file."
...
var renderers : Array[ORC_RendererBase]
...
func switch_active_renderer(new_renderer_idx : int) -> bool:
	if new_renderer_idx < NATIVE_RENDERER_IDX or new_renderer_idx >= renderers.size():
		ORC_Plugin.push_error(ERR_RENDERER_IDX_UNDEFINED % new_renderer_idx)
		return false
...
func _put_custom_renderer_offline(renderer_idx : int) -> void:
	...
	renderers[renderer_idx].cleanup()
...
func _put_custom_renderer_online(renderer_idx : int) -> void:
	...
	renderers[renderer_idx].scene_proxy.setup(scene)
...
func _process(delta: float) -> void:
	if Engine.is_editor_hint() || was_running_test_on_enter_tree:
		pass
	else:
