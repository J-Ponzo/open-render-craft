...
#define DECLARE_GD_OVERRIDABLE_METHOD(RET_TYPE, METHOD_NAME, ...) \
    ...
    virtual RET_TYPE METHOD_NAME##_impl(__VA_ARGS__);
...
RET_TYPE CLASS_NAME::METHOD_NAME() { \
...
RET_TYPE CLASS_NAME::METHOD_NAME(T1 A1) { \
    static const StringName override_name(#METHOD_NAME "_override"); \
    if (has_method(override_name)) { \
        if constexpr (std::is_same_v<RET_TYPE, void>) { \
        ...
        if constexpr (!std::is_same_v<RET_TYPE, void>) { \
            return METHOD_NAME##_impl(A1); \
        } else { \
            METHOD_NAME##_impl(A1); \
        } \
    } \
}
...
RET_TYPE CLASS_NAME::METHOD_NAME(T1 A1, T2 A2) { \
    static const StringName override_name(#METHOD_NAME "_override"); \
    if (has_method(override_name)) { \
        if constexpr (std::is_same_v<RET_TYPE, void>) { \
        ...
        if constexpr (!std::is_same_v<RET_TYPE, void>) { \
            return METHOD_NAME##_impl(A1, A2); \
        } else { \
            METHOD_NAME##_impl(A1, A2); \
        } \
    } \
}
...
RET_TYPE CLASS_NAME::METHOD_NAME(T1 A1, T2 A2, T3 A3) { \
    static const StringName override_name(#METHOD_NAME "_override"); \
    if (has_method(override_name)) { \
        if constexpr (std::is_same_v<RET_TYPE, void>) { \
        ...
        if constexpr (!std::is_same_v<RET_TYPE, void>) { \
            return METHOD_NAME##_impl(A1, A2, A3); \
        } else { \
            METHOD_NAME##_impl(A1, A2, A3); \
        } \
    } \
}
...
RET_TYPE CLASS_NAME::METHOD_NAME(T1 A1, T2 A2, T3 A3, T4 A4) { \
    static const StringName override_name(#METHOD_NAME "_override"); \
    if (has_method(override_name)) { \
        if constexpr (std::is_same_v<RET_TYPE, void>) { \
        ...
        if constexpr (!std::is_same_v<RET_TYPE, void>) { \
            return METHOD_NAME##_impl(A1, A2, A3, A4); \
        } else { \
            METHOD_NAME##_impl(A1, A2, A3, A4); \
        } \
    } \
}
